...
**Change**: `fulfillment-service/pkg/api/<domain>/<message>.proto`

**Related files to review**:
1. **gRPC service handler**: `fulfillment-service/internal/service/<domain>/<operation>.go`
   - Check: UpdateMask allowlist includes new field
   - Check: Validation logic for new field constraints
   
2. **OPA policies**: `fulfillment-service/pkg/authz/policies/<resource>.rego`
   - Check: Policy updated if new field affects authorization
   
3. **Integration tests**: `fulfillment-service/test/integration/<resource>_test.go`
...
5. **CLI commands**: `fulfillment-service/internal/cmd/osac/<resource>/*.go`
...
2. **Helm CRD copy**: `osac-installer/helm/osac-operator/crds/<crd>.yaml`
...
3. **RBAC markers vs Helm ClusterRole**:
   ...
   - Destination: `osac-installer/helm/osac-operator/templates/clusterrole.yaml`
   - Check: Rules match (verbs, resources, API groups)
   
4. **Ansible playbooks**: `osac-aap/playbooks/workflows/<resource>/*.yaml`
...
6. **Proto definition**: `fulfillment-service/pkg/api/<domain>/<message>.proto`
...
**Change**: `osac-aap/playbooks/workflows/<workflow>/*.yaml`
...
2. **Role implementations**: `osac-aap/roles/<role-name>/`
   - Check: Role variables documented in `defaults/main.yaml` or `README.md`
   
3. **Template roles**: `osac-aap/roles/<template-role>/templates/*.j2`
   - Check: Jinja2 templates use variables defined in playbook or role defaults
   
4. **Collection dependencies**: 
   - `osac-aap/collections/ansible_collections/osac/workflows/meta/runtime.yml`
   - `osac-aap/collections/ansible_collections/osac/config_as_code/meta/runtime.yml`
   - Check: New modules/plugins have collection dependencies declared
   
5. **Documentation**:
   - `osac-aap/CATALOG_ITEMS.md` if playbook provisions provider-defined resource
   - Role `README.md` if role variables or behavior changed

## When Helm Charts Change, Check These Files

### osac-installer Helm Change
**Change**: `osac-installer/helm/<component>/`

**Related files to review**:
1. **Component templates**: `osac-installer/helm/<component>/templates/*.yaml`
...
4. **Multi-component dependencies**: `osac-installer/helm/<component>/Chart.yaml`
...
**Change**: `fulfillment-service/migrations/<timestamp>_<name>.sql`

**Related files to review**:
1. **Model definitions**: `fulfillment-service/internal/storage/models/<resource>.go`
   - Check: Go struct tags match new columns (gorm, db)
   
2. **Query functions**: `fulfillment-service/internal/storage/queries/<resource>.sql` (if using sqlc)
   - Check: Queries updated to select/insert new columns
   
3. **Down migration**: Same file's `-- +goose Down` section
   - Check: Down migration reverses the change (drop column, drop index)
   
4. **Integration tests**: `fulfillment-service/test/integration/<resource>_test.go`
   - Check: Tests updated for new schema (if CRUD operations affected)

## When CLI Commands Change, Check These Files

### osac CLI Command
**Change**: `fulfillment-service/internal/cmd/osac/<command>/*.go`

**Related files to review**:
1. **Proto service**: `fulfillment-service/pkg/api/<domain>/<service>.proto`
...
3. **Parent command registration**: `fulfillment-service/internal/cmd/osac/root.go` or `<parent_cmd>.go`
   - Check: New command added as subcommand
   
4. **Integration tests**: `fulfillment-service/test/integration/cli_test.go`
...
- [ ] **AUTH.md** - new resources/verbs listed in tenant permissions table
- [ ] **architecture-patterns.md** - resource hierarchy diagram includes new types
- [ ] **CATALOG_ITEMS.md** - provider-defined resources documented
