...
**Related documents**:
- `INVARIANTS.md` - Architectural invariants and common bug patterns to check
...
2. Use INVARIANTS.md to check architectural invariants and common bug patterns
...
5. **UpdateMask bugs**:
   - When a field is added, is it in the UpdateMask allow-list?
...
- `// +kubebuilder:rbac:` markers in controller files must match Helm ClusterRole in `osac-installer/helm/osac-operator/templates/`

### OPA Policies
- Located in `fulfillment-service/pkg/authz/policies/`
- When a new tenant-scoped resource is added, check if OPA policies grant access
- `AUTH.md` must document the RBAC verbs (get/list/create/update/delete)

### Ansible Collections
- `osac.workflows` vs `osac.config_as_code` - see `osac-aap/AGENTS.md` for distinction
- Playbooks under `osac-aap/playbooks/workflows/` are ENTRY POINTS called by osac-operator
- Roles under `osac-aap/roles/` are IMPLEMENTATION called by workflow playbooks
...
File: docs/AUTH.md:47

The PR adds DiskImages CRD with create/read/update/delete verbs (pkg/authz/policies/disk_images.rego:15-23),
